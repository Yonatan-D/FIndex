...
const getAbsolutePath = (url, buckets = [], prefix = '/') => {
  const urlWithoutParams = decodeURIComponent(url).split("?")[0];

  const matchedBucket = buckets.length === 1
    ? buckets[0]
    : buckets.find(node => 
      urlWithoutParams.startsWith(`${prefix}${node.name}`)
    );

  if (!matchedBucket) {
    throw new Error(`未找到匹配的桶: ${urlWithoutParams}, 当前可用桶: ${buckets.map(b => b.name).join(', ')}`);
  }

  const relativePath = urlWithoutParams
    .replace(prefix, '')
    .replace(matchedBucket.name, '');
  const absolutePath = path.join(matchedBucket.path, relativePath.replace(/\/$/, ''));
  return { absolutePath, urlWithoutParams };
};

const createZipFile = (fileName, sourceDir) => {
...
export const createDownloadMiddleware = (config) => {
